...
class Db2ServerOfflineStoreConfig(FeastBaseModel):
    """Offline store config for SQL Server"""

    type: Literal["Db2", "feast_ibm_db2.db2_fs.Db2ServerOfflineStore"] = \
        "feast_ibm_db2.db2_fs.Db2ServerOfflineStore"
    """ Offline store type selector"""

    connection_string: StrictStr = "Db2+pyodbc://sa:yourStrong(!)Password@localhost:1433/feast_test?driver=ODBC+Driver+17+for+SQL+Server"
    """Connection string containing the host, port, and configuration parameters for SQL Server
     format: SQLAlchemy connection string, e.g. Db2+pyodbc://sa:yourStrong(!)Password@localhost:1433/feast_test?driver=ODBC+Driver+17+for+SQL+Server"""


def make_engine(config: Db2ServerOfflineStoreConfig) -> Engine:
    return create_engine(config.connection_string)


class Db2ServerOfflineStore(OfflineStore):
    """
    ...
    def pull_latest_from_table_or_query(
        ...
        assert type(data_source).__name__ == "Db2ServerSource"
        ...
        _start_date = start_date.strftime("%Y-%m-%d %H:%M:%S")
        _end_date = end_date.strftime("%Y-%m-%d %H:%M:%S")
        ...
        query = f"""
            ...
            FROM (
                ...
                WHERE {timestamp_field} BETWEEN TIMESTAMP_FORMAT('{_start_date}', 'YYYY-MM-DD HH24:MI:SS') AND TIMESTAMP_FORMAT('{_end_date}', 'YYYY-MM-DD HH24:MI:SS')
            ) outer_t
    ...
    def get_historical_features(
        ...
        if isinstance(entity_df, pandas.DataFrame):
            ...
            raise ValueError(
                "string entities are currently not supported in the Db2 offline store."
            )
        (
            table_schema,
            table_name,
...
class Db2ServerRetrievalJob(RetrievalJob):
    def __init__(
        self,
        query: str,
        engine: Engine,
        config: Db2ServerOfflineStoreConfig,
...
def _upload_entity_df_into_sqlserver_and_get_entity_schema(
    ...
    if type(entity_df) is str:
        ...
        limited_entity_df = Db2ServerRetrievalJob(
...
def _df_to_create_table_sql(df: pandas.DataFrame, table_name: str) -> str:
    ...
    columns = [f""""{f.name}" {pa_to_db2_type(f.type)}""" for f in pa_table.schema]
    print(columns)
    columns2 = [f""""{f.name}" {f.type}""" for f in pa_table.schema]
    print(columns2)
...
{{ featureview.name }}__base AS (
    ...
    INNER JOIN entity_dataframe
        ...
        {% if featureview.ttl == 0 %}{% else %}
        AND {{ featureview.ttl }} >= TIMESTAMPDIFF(2, char(subquery.event_timestamp - entity_dataframe.{{entity_df_event_timestamp_col}}))
        {% endif %}
